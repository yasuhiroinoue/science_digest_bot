...
HEADERS = {'User-Agent': 'DOIScraperBot/1.0 (mailto:your-email@example.com)'} # Use email from .env?
...
def get_crossref_abstract(doi: str, headers: dict) -> str | None:
    ...
    try:
        ...
        if message:
            ...
            print(f"Could not find 'message' key in Crossref API response for DOI {doi}.")
            return None
...
def get_pubmed_abstract(doi: str, headers: dict) -> str | None:
    ...
    esearch_params = {
        ...
        "tool": "DOIScraperBot", "email": email
    }
    ...
    if not pmid: return None # Exit if no PMID found
...
def main():
    ...
    for article in articles: # Process all articles
        ...
        if abstract and isinstance(abstract, str):
            ...
            if abstract.strip().startswith('<'):
                 ...
                 if cleaned_abstract:
                     ...
                     abstract = "Abstract found but empty after cleaning." # Or keep original?
        ...
        if abstract is None:
            ...
            article['summary'] = "Abstract not found." # Use the default message for Gemini
        else:
